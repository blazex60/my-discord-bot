...
| `collector.py` | DBおよびDiscord APIからのメッセージ取得ロジック |
| `llm_client.py` | llama-cpp-python HTTP サーバーへのリクエストラッパー |
| `db.py` | SQLite操作・TTL管理・メッセージ保存/削除 |
| `scheduler.py` | 定期自動投稿のスケジュール管理 |
| `config.yaml` | TTL・LLM URL・監視設定など |
| `.env.example` | DISCORD_TOKEN等のシークレットテンプレート |
| `pyproject.toml` | Python依存関係（uv管理） |
| `docker-compose.yml` | BotサービスとLLMサーバーの2サービス構成 |
| `Dockerfile` | Botコンテナのビルド定義 |
...
| Directory | Purpose |
|-----------|---------|
| `models/` | Gemma4 26B GGUFモデルファイル置き場（.gitignore対象） |
| `data/` | SQLiteデータベースファイル置き場（.gitignore対象） |
| `tests/` | ユニット・統合テスト |
