...
func guildDelete(s *discordgo.Session, g *discordgo.GuildDelete) {
	...
	adminRoleCount, err := service.ClearGuildAdminRole(g.ID)
	if err != nil {
		logger.Error("Failed to clean up guild data", "error", err, "guild_id", g.ID, "type", "admin-role")
	}
...
func rankLabel(rank int) string {
	if rank <= len(medals) {
		return fmt.Sprintf("%s 第%d位", medals[rank-1], rank)
	}
...
func handleRankCommand(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	for _, rank := range ranks {
		...
		embed.Fields = append(embed.Fields, &discordgo.MessageEmbedField{
			Name:   fmt.Sprintf("%s: %d回", rankLabel(rank.Rank), rank.Count),
			Value:  displayName,
			Inline: true,
		})
	}
