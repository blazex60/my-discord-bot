	...
	return []*discordgo.ApplicationCommand{
		{
			...
			Options: []*discordgo.ApplicationCommandOption{
				...
				{
					Name:        "role-set",
					Description: "このサーバーの管理者ロールを設定します",
					Type:        discordgo.ApplicationCommandOptionSubCommand,
					Options: []*discordgo.ApplicationCommandOption{
						{
							Name:        "role",
							Description: "管理者として設定するロール",
				...
				{
					Name:        "role-unset",
					Description: "このサーバーの管理者ロール設定を削除します",
					Type:        discordgo.ApplicationCommandOptionSubCommand,
				},
				{
					Name:        "role-show",
					Description: "このサーバーの現在の管理者ロール設定を表示します",
					Type:        discordgo.ApplicationCommandOptionSubCommand,
				},
			},
		},
...
func HandleAdminCommand(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	case "role-set":
		handleAdminRoleSet(s, i, options[0].Options)
	case "role-unset":
		handleAdminRoleUnset(s, i)
	case "role-show":
		handleAdminRoleShow(s, i)
	}
}
...
// isServerAdminOrRole checks if the user is a server admin (PermissionAdministrator)
// or holds the guild-configured admin role. Falls back to PermissionAdministrator only
// when no role is configured (backward compatible).
func isServerAdminOrRole(i *discordgo.InteractionCreate) bool {
	if isServerAdmin(i) {
		return true
	}
	...
	roleID, err := service.GetGuildAdminRole(i.GuildID)
	if err != nil {
		logger.Warn("admin role lookup failed; denying", "error", err, "guild_id", i.GuildID)
		return false
	}
	if roleID == "" {
		return false
...
func handleAdminRoleSet(s *discordgo.Session, i *discordgo.InteractionCreate, options []*discordgo.ApplicationCommandInteractionDataOption) {
	if i.GuildID == "" {
	...
	if role == nil {
		respondError(s, i, "ロールの取得に失敗しました")
		return
	}
	if err := service.SetGuildAdminRole(i.GuildID, role.ID); err != nil {
		logger.Error("Failed to set guild admin role", "error", err)
		respondError(s, i, "管理者ロールの設定に失敗しました")
		return
	}
	respondEphemeral(s, i, fmt.Sprintf("管理者ロールを <@&%s> に設定しました ✅", role.ID))
}

func handleAdminRoleUnset(s *discordgo.Session, i *discordgo.InteractionCreate) {
	if i.GuildID == "" {
		respondError(s, i, "このコマンドはサーバー内でのみ使用できます")
		return
	}
	if _, err := service.ClearGuildAdminRole(i.GuildID); err != nil {
		logger.Error("Failed to clear guild admin role", "error", err)
		respondError(s, i, "管理者ロール設定の削除に失敗しました")
		return
	}
	respondEphemeral(s, i, "管理者ロール設定を削除しました ✅")
}

func handleAdminRoleShow(s *discordgo.Session, i *discordgo.InteractionCreate) {
	if i.GuildID == "" {
		respondError(s, i, "このコマンドはサーバー内でのみ使用できます")
		return
	}
	roleID, err := service.GetGuildAdminRole(i.GuildID)
	if err != nil {
		logger.Error("Failed to get guild admin role", "error", err)
		respondError(s, i, "管理者ロール設定の取得に失敗しました")
		return
	}
	if roleID == "" {
		respondEphemeral(s, i, "管理者ロールは設定されていません（PermissionAdministratorのみ有効）")
		return
	}
	respondEphemeral(s, i, fmt.Sprintf("現在の管理者ロール: <@&%s>", roleID))
}
